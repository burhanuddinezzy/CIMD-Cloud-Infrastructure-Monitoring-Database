# This file also will be running non stop but this file has the functions that dont neeed to be running as often as the main mock data file that has all the server metrics, etc.
# nano mockdataslow.py
# python3 mockdataslow.py
# rm mockdataslow.py
# crontab -
# 0 0,12 * * * /usr/bin/python3 /path/to/mockdataslow.py
...
def main():
    ...
    try:
            with get_conn() as conn, conn.cursor() as cur:
                # Pick a random location_id from the location table
                ...
                # Insert cost data for each server
                timestamp = datetime.now(GMT_PLUS_4)
                for _ in range(random.randint(3, 7)):
                    insert_users(cur, location_id)                
                ...
                conn.commit()
                print(f"Inserted cost data for {len(SERVER_IDS)} servers at {timestamp}.")
